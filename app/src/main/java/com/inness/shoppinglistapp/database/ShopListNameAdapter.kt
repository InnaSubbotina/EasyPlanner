...
class ShopListNameAdapter(private val listener: Listener) :
    ...
    class ItemHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun setData(shopListNameItem: ShopListNameItem, listener: Listener) = with(binding) {
            ...
            tvTimeShop.text = shopListNameItem.time
            progBar.max = shopListNameItem.allItemCounter
            progBar.progress = shopListNameItem.checkedItemsCounter
            val colorState = ColorStateList.valueOf(
                getProgressColorState(shopListNameItem, binding.root.context))
            progBar.progressTintList = colorState
            ibCounterCard.backgroundTintList = colorState
            ...
            itemView.setOnLongClickListener {
                shopListNameItem.id?.let { it1 -> listener.deleteItem(it1) }
                true
            }
            ibEditshop.setOnClickListener {
        ...
        private fun getProgressColorState(item: ShopListNameItem,context: Context) : Int {
             return if(item.checkedItemsCounter == item.allItemCounter){
                 ContextCompat.getColor(context,R.color.progress_bar_green)
             } else {
                 ContextCompat.getColor(context,R.color.progress_bar_blue)
             }
        }
            ...
            fun create(parent: ViewGroup): ItemHolder {
                return ItemHolder(
                    LayoutInflater.from(parent.context)
                        .inflate(R.layout.list_shopname_item, parent, false))
            }
        }
    }
